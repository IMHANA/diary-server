...
export class DiaryService {
  ...
  //일기 일별 조회
  async getDiaryWithDate(day: string): Promise<diary[]> {
    const data = await this.prisma.$queryRaw(
      `select * from diary where to_char(diary_date, 'DD') = '${day}'
      and to_char(diary_date, 'MM') = '${month}'
      and to_char(diary_date, 'YYYY') = '${year}';`,
    );
    return data;
  }
